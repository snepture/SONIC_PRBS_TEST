...
@click.option(
    "--prbs_test_start","prbs_test_start",
    is_flag = True,
    help = "define the status to start"
)
@click.option(
    "--prbs_test_stop","prbs_test_stop",
    is_flag = True,
...
@coro
async def prbs_test_show(interface, prbs_status):
    socketclient = await initsock()
    slice, ifg, serdes = port2serdes(interface)
    if prbs_status:
...
prbs_test.add_command(prbs_test_show,name="prbs_show")
